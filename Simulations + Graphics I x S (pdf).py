...
SxI_pdf = 'SxI para N=1e7 (t0 = 80).pdf'

# Definindo x e y para plotar a reta S + I + R = 1
x = np.linspace(0, 1, 1000)
y = 1 - x

with PdfPages(SxI_pdf) as pdf:
    for i in range(len(R0)):
        ...
        r0_atual = R0[i]
        plt.plot(susceptibles, infected)
        plt.plot(x, y, 'k--', label='S + I + R = 1', linewidth=0.5)
        plt.legend()
        plt.xlabel('Susceptibles Ratio')
        plt.ylabel('Infected Ratio')
        plt.grid(True)
        plt.title(f'S x I for R= {r0_atual} (N = 1e7)', fontsize=18)
        params_legend_text = (
        f'Parameters:\n'
        f'vac_num = {vac_num}\n'
        f'$\\gamma$ = {GAMMA}\n'
    ...
    print(f"All plots saved to {SxI_pdf}.")
