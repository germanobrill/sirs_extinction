...
t_total = 3000.0 # Duração máxima das simulações
...
def simular_uma_vez(n, r0, gamma, delta, vac_num, t0, a):
    ...
    params_dict = {'n': n, 'r0': r0, 'gamma': gamma, 'delta': delta, 'vac_num': vac_num, 't0': t0, 'a': a, 
...
R0 =  np.array([2])# Número de reprodução básico
GAMMA = np.array([0.2]) # Taxa de cura
DELTA = np.array([0.01]) # Taxa de perda de imunidade
VAC_NUM = np.array([0.3]) # Número de vacinas disponíveis (Normalizado)
T0 = np.arange(25, 125, 1) # Data de início da vacinação
A = np.arange([0.01]) #Taxa de vacinação
...
with h5py.File('Grafico Pablo.h5', 'w') as f:
